...
def correct():
    log("####################### image Correction started ######################")
    processed_dir = 'C:\\Users\\Gaurav\\Desktop\\DIP_Project\\Image_DataSet_100_UnProcessed'
    root_dir = 'C:\\Users\\Gaurav\\Desktop\\DIP_Project\\Corrected image'
    ...
    log("Correction Script started at :: ")
    for cls in classes_dir:
        ...
        for name in allFileNames:
            img_path = src + '\\' + name
            final_path = root_dir + '\\' +cls + '\\' + name
            img = imageio.imread(img_path)
            try:
                result = Ying_2017_CAIP(img)
                cv2.imwrite(final_path, cv2.cvtColor(result, cv2.COLOR_RGB2BGR))
            except Exception as e:
                error += 1
                log("Error while correcting image: " + str(e) + " " + name + "Image Path: " + img_path)
                cv2.imwrite(final_path, cv2.cvtColor(img, cv2.COLOR_RGB2BGR))

    log("Script Ended at :: ")
    
    log("###################### images Successfully Color corrected and saved at Corrected Images Folder ###################")
